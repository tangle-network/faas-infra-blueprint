...
async fn test_real_docker_execution() {
    println!("🔍 VERIFYING: Real Docker Execution Test");
    ...
    println!("   Container creation + execution should take >100ms");

    assert!(result.is_ok());
    let response = result.unwrap().response;
    println!("   Response: {:?}", response);
    assert_eq!(response, Some(b"hello\n".to_vec()));

    if duration.as_millis() < 50 {
        println!("   ⚠️ WARNING: Execution under 50ms suggests caching or pooling!");
    } else {
        println!("   ✅ Execution time looks legitimate");
    }
}
...
async fn test_container_pool_warm_start() {
    ...
    // Pre-warm the pool and wait for it to actually complete
    let pool = pool_manager.get_pool("alpine:latest").await;

    // Manually trigger pre-warming and wait for completion
    pool.pre_warm().await.unwrap();

    // Give additional time for containers to be fully ready
