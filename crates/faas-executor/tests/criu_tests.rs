...
impl MockCriuManager {
    ...
    pub async fn checkpoint(
        ...
        self.snapshots.write().await.insert(snapshot_id, snapshot.clone());
        Ok(snapshot)
    }
    ...
    pub async fn restore(
        ...
        self.containers.write().await.insert(new_container_id, restored_state.clone());
        Ok(restored_state)
    }
    ...
    pub async fn create_branch(
        ...
        drop(snapshots);
        self.snapshots.write().await.insert(branch_id.clone(), branch_snapshot);
        Ok(branch_id)
    }
    ...
    pub async fn merge_branches(
        ...
        let snapshots = self.snapshots.read().await;
        let source = snapshots.get(source_id)
            .ok_or_else(|| anyhow::anyhow!("Source branch not found"))?;
        let target = snapshots.get(target_id)
        ...
        for (path, content) in &source.container_state.file_system {
            merged_state.file_system.insert(path.clone(), content.clone());
        }
        ...
        drop(snapshots);
        self.snapshots.write().await.insert(merged_id.clone(), merged_snapshot);
        Ok(merged_id)
    }

    pub async fn get_snapshot_size(&self, snapshot_id: &str) -> Option<u64> {
        self.snapshots.read().await.get(snapshot_id).map(|s| s.size_bytes)
    }
...
mod tests {
    ...
    async fn test_create_and_restore_snapshot() {
        ...
        let restored = criu.restore("snap-1", "restored-container".to_string())
            .await
            .unwrap();
    ...
    async fn test_incremental_snapshots() {
        ...
        // Restore and modify
        let mut restored = criu.restore("snap-base", "modified".to_string())
            .await
            .unwrap();
        
        // Simulate modifications
        restored.file_system.insert("/app/data.txt".to_string(), b"new data".to_vec());
        restored.memory_usage += 50_000_000;
        
        // Create incremental snapshot
        let incremental_snap = criu.checkpoint(
            "modified",
        ...
        assert_eq!(incremental_snap.parent_snapshot, Some("snap-base".to_string()));
    }
    ...
    async fn test_branching_and_merging() {
        ...
        let feature_branch = criu.create_branch(
            "snap-main",
        ...
        let mut feature_state = criu.restore("branch-feature", "feature-container".to_string())
            .await
            .unwrap();
        ...
        feature_state.env_vars.insert(
            "FEATURE_FLAG".to_string(),
            "enabled".to_string(),
        );
        
        // Create another branch for testing
        let test_branch = criu.create_branch(
            "snap-main",
            "branch-test".to_string(),
        ).await.unwrap();
        
        // Merge feature into test
        let merged = criu.merge_branches(
            "branch-feature",
            "branch-test",
        ).await.unwrap();
    ...
    async fn test_snapshot_size_tracking() {
        ...
        criu.checkpoint("test", "snap-2".to_string(), true, Some("snap-1".to_string()))
            .await
            .unwrap();
        
        criu.checkpoint("test", "snap-3".to_string(), true, Some("snap-2".to_string()))
            .await
            .unwrap();
    ...
    async fn test_snapshot_restore_state_consistency() {
        ...
        for i in 0..3 {
            ...
            assert!(restored.file_system.contains_key("/var/lib/postgresql/data/db.sql"));
